 ...
 * `on duplicate key update` clause in Snowflake. It's a literal copy of the MySQL version.
 *
...
public fun <T : BaseTable<*>> Database.insertOrUpdate(
    ...
    require(builder.assignments.isNotEmpty()) { "There are no columns to insert in the statement." }
 ...
 */
@KtormDsl
public open class SnowflakeAssignmentsBuilder : AssignmentsBuilder() {
 ...
 */
@KtormDsl
public class InsertOrUpdateStatementBuilder : SnowflakeAssignmentsBuilder() {
    ...
    public fun onDuplicateKey(block: AssignmentsBuilder.() -> Unit) {
        val builder = SnowflakeAssignmentsBuilder().apply(block)
