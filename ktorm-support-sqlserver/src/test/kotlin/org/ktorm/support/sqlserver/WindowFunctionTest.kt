...
class WindowFunctionTest : BaseSqlServerTest() {
    ...
    fun testAvg() {
        ...
        assertEquals(setOf("vince:112.00", "marry:112.00", "tom:112.00", "penny:112.00"), results.toSet())
    }
    ...
    fun testRunningAvg() {
        ...
        assertEquals(setOf("vince:100.00", "marry:75.00", "tom:116.00", "penny:112.00"), results.toSet())
    }
    ...
    fun testRange() {
        val results = database
            ...
            .select(
                Employees.name,
                sum(Employees.salary).over { orderBy(Employees.salary.asc()).range(WindowFrames.unboundedPreceding()) }
            )
            .map { row ->
                "${row.getString(1)}:${row.getLong(2)}"
            }

        assertEquals(setOf("marry:50", "vince:250", "penny:250", "tom:450"), results.toSet())
    }
