...
fun <T : Table<*>> T.update(block: UpdateStatementBuilder.(T) -> Unit): Int {
    val builder = UpdateStatementBuilder().apply { block(this@update) }
    val expression = AliasRemover.visit(UpdateExpression(asExpression(), builder.assignments, builder.where))
...
fun <T : Table<*>> T.insert(block: AssignmentsBuilder.(T) -> Unit): Int {
    val builder = AssignmentsBuilder().apply { block(this@insert) }
    val expression = AliasRemover.visit(InsertExpression(asExpression(), builder.assignments))
...
fun <T : Table<*>> T.insertAndGenerateKey(block: AssignmentsBuilder.(T) -> Unit): Any? {
    val builder = AssignmentsBuilder().apply { block(this@insertAndGenerateKey) }
    val expression = AliasRemover.visit(InsertExpression(asExpression(), builder.assignments))
...
open class AssignmentsBuilder {
    internal val assignments = ArrayList<ColumnAssignmentExpression<*>>()
...
class BatchUpdateStatementBuilder<T : Table<*>>(internal val table: T) {
    ...
    fun item(block: UpdateStatementBuilder.(T) -> Unit) {
        ...
        val expr = UpdateExpression(table.asExpression(), builder.assignments, builder.where)
...
class BatchInsertStatementBuilder<T : Table<*>>(internal val table: T) {
    ...
    fun item(block: AssignmentsBuilder.(T) -> Unit) {
        ...
        val expr = InsertExpression(table.asExpression(), builder.assignments)
